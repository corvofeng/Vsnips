...
describe("Tokenize", () => {
  ...
  it("Parse typesctipt function", () => {
    ...
    TEST_JS_AND_TS_FUNCS.forEach((c) => {
      const tok = parseTokenizer(c[0][0] as string, c[0][1] as string);
      Logger.debug("Get tokobj: ", tok);

      if (c.length === 2) {
        const expectToken = c[1][0] as TsFuncToken;
        if (tok !== undefined) {
  ...
  it("Parse python function", () => {
    ...
    TEST_FUNCS.forEach((c) => {
      const tok = parseTokenizer(c[0][0] as string, c[0][1] as string) as PyFuncToken;
      if (c.length === 2) {
        const expectToken = c[1][0] as PyFuncToken;
  ...
  x2 int) {`, "golang"],
    ...
    TEST_JS_AND_TS_FUNCS.forEach((c) => {
      const tok = parseTokenizer(c[0][0] as string, c[0][1] as string) as GoFuncToken;
      Logger.debug("Get tokobj: ", tok);
      if (c.length === 2) {
        const expectToken = c[1][0] as GoFuncToken;
        if (expectToken !== undefined) {
          Logger.debug("Wanna get ", expectToken, "get", tok);
